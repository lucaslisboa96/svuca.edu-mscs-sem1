...
const_32f: 		32.0
.word 
const_studentid:	150201133 	#my student ID, in integer :) 
const_36:		36		
const_24:		24
const_101:		101
const_59:		59		
...
main:	
	...
	jal 	c2f			# call c2f, once completes, result stored in $f0 
	mov.s	$f2, $f0		# copy & store result of c2f into f2, for verification

	#######################################################
	## - Compress student ID to [60, 100] 
	## - Convert new student ID from Fahrenheit to Celcius
	## - RESULT stored in $f4
	#######################################################
	lw	$t0, const_studentid	# prepare input, load student_id to $t0
	...
	jal 	f2c			# call c2f, once completes, result stored in $f0 
...
compress_student_id: #input: $t0, $t5, $t6, output: $f0
_LOOP:
	srl 	$t0, $t0, 1		# t0 = t0 / 2
